...
export default function FilterCallsType() {
  ...
  const [choice, setChoice] = useState("Все типы");
  const handleClick = (e) => setChoice(e.target.innerText);

  const handleIncomingCalls = (e) => {
    setChoice(e.target.innerText);
    dispatch(getIncomingCalls("&in_out=1"));
  };

  const handleOutcomingCalls = (e) => {
    setChoice(e.target.innerText);
    dispatch(getOutcomingCalls("&in_out=0"));
  };

  const handleAllCalls = (e) => {
    setChoice(e.target.innerText);
    dispatch(getAllCallsType());
  };

  return (
    <div className="filter-calls-type">
      <div className="filter-calls-type__default">
      ...
      <div className="filter-calls-type__choice">
        <div className="choice__types">
          <div className="types__item" onClick={handleAllCalls}>
            Все типы
          </div>
          <div className="types__item" onClick={handleIncomingCalls}>
            Входящие
          </div>
          <div className="types__item" onClick={handleOutcomingCalls}>
            Исходящие
          </div>
        </div>
      </div>
