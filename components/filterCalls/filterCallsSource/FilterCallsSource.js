...
import { setFilterStatusSource } from "@/redux/features/callsSlice";
import { useDispatch, useSelector } from "react-redux";

export default function FilterCallsSource() {
  const [choice, setChoice] = useState("Все источники");
  ...
  const handleClick = (e) => {
    ...
    setFilter(e.target.innerText === "Все источники" ? false : true);
  };

  useEffect(() => {
    dispatch(setFilterStatusSource(filter));
  }, [filter]);
  ...
  return (
    <div className="filter-calls-source">
      <div
        className={clsx("filter-calls-source__default", {
          "filter-calls-source__default--active": filter,
        })}
      >
